...
export class DetectionEngine {
  ...
  _renderLoop() {
    ...
    if (this.fpsTimes.length >= 2) {
      ...
      if (fpsEl) fpsEl.textContent = Math.round(((this.fpsTimes.length-1)/elapsed)*1000);
    }
  ...
  _analyzeFrame() {
    ...
    const curr = this.offCtx.getImageData(0, 0, w, h).data;
    ...
    if (changes / (w * h) > 0.005) {
      const blobs = this._findBlobs(diff, w, h);
      const scale = this.canvas.width / w;
      this._lastBlobs = blobs
        .map(b => ({ x: b.x*scale, y: b.y*scale, w: b.w*scale, h: b.h*scale, area: b.area*scale*scale }))
        .filter(b => b.area > 800);
  ...
  _drawBoxes(blobs) {
    ...
    for (const b of blobs) {
      const bx = b.x+8, by = b.y+8, bw = b.w-16, bh = b.h-16, len = 14;
      ctx.strokeStyle = "#00ff9d"; ctx.lineWidth = 1.5; ctx.shadowColor = "#00ff9d"; ctx.shadowBlur = 10;
      ...
      ctx.shadowBlur = 0; ctx.fillStyle = "rgba(0,0,0,0.75)"; ctx.fillRect(bx, by-18, 72, 16);
      ctx.fillStyle = "#00ff9d"; ctx.font = "bold 9px 'Space Mono', monospace"; ctx.fillText("MOTION", bx+4, by-5);
    }
    ctx.restore();
  }
  ...
  _captureSnapshot() {
    ...
    this.ringBuffer = this.ringBuffer.filter(f => f.timestamp >= now - RING_BUFFER_MS);
  }
  ...
  _closeDetection() {
    ...
    const preview  = this.ringBuffer.length > 0
      ...
      : null;
    const clip = { startTimestamp: this.detectionStart, endTimestamp: endTime, durationSeconds: duration, frameCount: this.ringBuffer.length, previewFrame: preview };
...
export async function setOwnerOnline() {
  ...
  onValue(connRef, async snap => {
    if (snap.val() === true) {
      try {
        await onDisconnect(liveRef).set({ online: false, updatedAt: Date.now() });
        await set(liveRef, { online: true, updatedAt: Date.now() });
